...
if [[ $# -ne 8 ]] ; then
    ...
    echo ""
    echo "Usage: ./physical-greenfield-option2a.sh [DeviceBucket] [DeviceKeyGreengrass] [RestAPIId] [Region] [EdgeDeviceID] [hardwarePublicIP] [DeviceKeyAwareDevice] [IoTEndpoint]"
    echo ""
    exit 1
fi
