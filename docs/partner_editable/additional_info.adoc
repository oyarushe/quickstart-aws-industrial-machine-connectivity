...
The following directories and files are necessary for running an IMC kit deployment: 
...
* *functions/*: Contains zipped lambda code that is used for various pieces of the IMC kit.
...
==== Setting up a QuickSight dashboard along with the IMC Kit deployment

After the IMC Kit deployment succeeds (models and assets have finished creation in AWS IoT SiteWise), the following manual steps are required to create a QuickSight dataset. 

. Allow QuickSight access to the S3 bucket that houses the data from the IMC Kit. 
...
Find the S3 bucket labeled `[IMCKitDeploymentName]...-imcs3bucket-….`, click the checkbox on the left side, and click “Finish”.
...
. Download the public IMC Kit AWS Quick Start Github repo (https://github.com/aws-quickstart/quickstart-aws-industrial-machine-connectivity) as a zip file.
